...
export const vipApi = {
  // 获取念念不忘会员列表
  getNeverForgetVips: async (userId, page = 1, size = 10, sortBy = null, sortDir = null) => {
    let url = `/api/vips/never-forget/${userId}?page=${page}&size=${size}`;
  ...
  // 获取好久不见会员列表
  getLongTimeNoSeeVips: async (userId, page = 1, size = 10, sortBy = null, sortDir = null) => {
    let url = `/api/vips/long-time-no-see/${userId}?page=${page}&size=${size}`;
    if (sortBy) url += `&sortBy=${sortBy}`;
    if (sortDir) url += `&sortDir=${sortDir}`;
    const response = await axios.get(url);
    return handleResponse(response);
  ...
  getNeverForgetVipsCount: async (userId) => {
    const response = await axios.get(`/api/vips/never-forget-count/${userId}`);
    return handleResponse(response);
  },
  
  // 获取好久不见会员数量
  getLongTimeNoSeeVipsCount: async (userId) => {
    const response = await axios.get(`/api/vips/long-time-no-see-count/${userId}`);
    return handleResponse(response);
  },
  
  // 获取所有VIP数量
  getAllVipsCount: async () => {
    const response = await axios.get('/api/vips/count');
    return handleResponse(response);
  },
  ...
  getVipsByUserId: async (userId) => {
    const response = await axios.get(`/api/vips/user/${userId}`);
    return handleResponse(response);
  },
  
  // 根据用户ID获取VIP列表 (分页)
  getVipsByUserIdPaged: async (userId, page = 1, size = 10, sortBy = null, sortDir = null) => {
    let url = `/api/vips/user/${userId}/paged?page=${page}&size=${size}`;
    if (sortBy) url += `&sortBy=${sortBy}`;
    if (sortDir) url += `&sortDir=${sortDir}`;
    const response = await axios.get(url);
    return handleResponse(response);
