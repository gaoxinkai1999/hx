              ...
              <el-table
                ...
                <el-table-column label="操作">
                  <template slot-scope="scope">
                    <el-button
                        size="mini"
                        @click="choose_vip(scope.$index, scope.row)">选择</el-button>
                  </template>
                </el-table-column>
              </el-table>
            </el-dialog>
...
export default {
  ...
  methods: {
    ...
    handleNodeClick(data) {
     ...
     async get念念不忘() {
       var res = await this.$http.post('念念不忘', {UserId: this.NodeUser.id});
       this.vips = res.data


     },
    async get好久不见() {
      var res = await this.$http.post('好久不见', {UserId: this.NodeUser.id});
      this.vips = res.data
